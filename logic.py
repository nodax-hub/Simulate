...
# Преобразование Гео координат в декартовы на плоскости
...
# --- реализация: эквидистантная проекция (сферическая модель) ---
...
# --- реализация: азимутальная эквидистантная проекция ---
...
class PumpController:
    ...
    def compute_flow_series(self, t, v_motion, length: float, volume_total: float, eps=1e-9) -> PumpPlan:
        ...
        """
        плотность = v_total / length
        
        q(t) = плотность * speed_motion(t)
        """
    ...
    @staticmethod
    def instantaneous_densities(
            v_motion: np.ndarray,  # м/с, shape (T,)
            v_pump: np.ndarray,  # л/с, shape (T,)
            W: Optional[float] = None,  # м, ширина захвата; если None, считаем только линейную
            eps: float = 1e-9
    ) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        """
        Возвращает:
          lambda_lperm (л/м), A_lperha (л/га) или None, если W не задана.
        ...
        # где движемся:
        moving = ~stopped
        lambda_lperm[moving] = v_pump[moving] / v_motion[moving]
        ...
        A_lperha = None
        if W is not None:
            A_lperha = lambda_lperm / max(W, eps) * 1e4  # л/м * (1/м) * 10^4 = л/га

        return lambda_lperm, A_lperha
